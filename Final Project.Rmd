...
Based on the decision of the mouse, the researchers would provide either a reward or a penalty. The visual stimuli varied in terms of contrast levels (0, 0.25, 0.5, or 1) and direction (left or right) recorded in the `contrast_left` and `contrast_right` variables. A successful trial is categorized by the mouse:
...
brain.areas <- sort(brain.areas) # sort alphabetically, acknowledgements 7
...
for (i in 1:18){
  ...
  for (j in 1:length(brain.areas)){
    ...
    sessions.brain.areas.counts <- sessions.brain.areas.counts %>% 
      add_row(Session=session.id, Mouse=mouse, Brain.Area=ba, Avg.Count=avg.count) # acknowledgements 5
  }
}
...
sessions.brain.areas.counts %>% ggplot() +
  ...
  scale_x_discrete(guide = guide_axis(n.dodge = 2)) + # acknowledgements 6
  scale_y_continuous(guide=guide_axis(n.dodge = 1)) + # acknowledgements 6
  coord_flip() +
...
for (i in 1:18){
  ...
  for (j in 1:length(brain.areas)){
    ...
    sessions.brain.areas.obs <- sessions.brain.areas.obs %>% 
      add_row(Session=session.id, Mouse=mouse, Brain.Area=ba, Count=count) # acknowledgements 5
  }
}
...
for (i in c(2, 6, 9, 17)) {
  ...
  for (j in 1:nrow(sessions[[i]]$feedback_type)) {
    ...
    # add row to tibble, acknowledgements 5
...
sessions.summary[,brain.areas] = NA # acknowledgements 8
for (i in 1:nrow(sessions.summary)) {
  s.i = sessions.summary[i,]$Session
  t.i = sessions.summary[i,]$Trial
...
```{r, echo=FALSE}
s4.ba <- sessions[[4]]$brain_area %>% unique() %>% sort() # acknowledgements 7
s16.ba <- sessions[[16]]$brain_area %>% unique() %>% sort() # acknowledgements 7
...
for (i in 1:nrow(sessions.summary)){
  ...
  max.spk.index = which.max(summed.spks) # acknowledgements 4
...
for(i in 1:18){
  ...
  for(j in 1:n.trials){
    ...
    max.spk = summed.spk.matrix[which.max(summed.spk.matrix)] # acknowledgements 4
...
Now that I have integrated the relevant information across all sessions and trials into one data table, I can begin building and evaluating prediction models. In this section, I will build a variety of different models and compare their performances to decide upon the optimal model for predicting trial outcome. First, I will split my data into a training and testing sets so that I am able to avoid over-fitting my models and evaluate their prediction accuracy against the test set. 

## Split-Sample Preparation

I will split my integrated data set into a training and testing set so that I can build and evaluate the performance of my prediction models. Before splitting my data, I first changed the `Feedback` column into a factor with levels 0 and 1, for unsuccessful and successful trials, respectively. Then, I also scaled my data so that it is normalized. 
...
Now that my data has been integrated across all sessions and trials, I will begin building and evaluating models based on their ability to accurately predict the trial outcomes. I will be evaluating the performance of various models based on three model metrics described below, which will then allow me to compare the performance of all the models and help me select the best one. 
...
Now that I have defined the measurements by which I will be evaluating the performance of the predictive models, I will define a benchmark model against which I can compare my other models against to ensure they have improved their accuracy in making trial outcome predictions.

## Benchmark Models

For the benchmark models, I will only be using only the trial number as a predictor as this was a variable given to us in the data set. I decided on using a LDA model and a Random Forest model for my two benchmark models as they are two different classification methods with different levels of complexity. LDA looks at linear combinations to classify the data whereas Random Forest uses multiple decision trees to classify the data, making it more intensive than LDA. By including a more simple LDA model and a more complex Random Forest model for my benchmark models, I am able to better assess whether my choice of additional predictors actually improves the classification ability of the models. 

As I build my models, I will be inputting the results of the evaluation criteria mentioned above into a summary table, which I will then use to decide upon the optimal predictive model. For each model I will print the accuracy rate just to get a quick idea of how the model performs. The other evaluation criteria will be viewable in the summary table of all fitted models. 
...
# add to summary table,  acknowledgements 5
...
With the benchmark models defined, I can create models with the additional predictors: `Session`, `Trial`, `Mouse`, `Con.L`, `Con.R`, `Avg.Spikes`, `Max.Spike`, and the interaction term of `Con.L*Con.R`.

I then evaluate their performance, and compare them to the benchmark model. The first model I fit to the data was a Linear Discriminant Analysis model.
...
# train model
mod.lda <- lda(formula=Feedback ~., data=train.data)
#mod.lda
...
me = misclass.rate(cm)
f1 = F1(cm)
print("LDA without Interaction or Root Spike:")
cat("Model Accuracy:", acc)

# add to summary table, acknowledgements 5
model.sum <- model.sum %>% add_row(Model="LDA, no interaction or root", 
                                   Accuracy=acc, Misclassfication.Rate=me, F1.Score=f1)
```

In the LDA model above, I used the `Mouse`, `Trial`, `Avg.Spikes`, `Con.L`, and `Con.R` predictors in my model based on the visualizations comparing these values between the successful and unsuccessful trials above. I found that omitting the `Con.L:Con.R` interaction term actually increased my model's accuracy, so I left it out. 
...
```{r, echo=FALSE}
# train model
mod.logr <- glm(formula=Feedback~ ., family="binomial", data=train.data)
#summary(mod.logr)
...
acc = accuracy.rate(cm)
f1 = F1(cm)
print("Logistic Regression without Interaction or Root Spike:")
cat("Model Accuracy:", acc)

# add to summary table, acknowledgements 5
model.sum <- model.sum %>% add_row(Model="Logistic Regression, no interaction or root", Accuracy=acc,
                                   Misclassfication.Rate=me, F1.Score=f1)
```

I built a logistical regression model using `Session`, `Mouse`, `Trial`, `Avg.Spikes`,`Max.Spike`, and the `Con.L` and `Con.R` variables as predictors. Here also, I found that omitting the `Con.L:Con.R` interaction term produced a more accurate model, so I dropped this term from the model. 
...
Before fitting my kNN model, I first found the optimal k using a cross-validation approach of fitting various values of k and choosing the one that minimized the misclassifaction rate.

```{r, echo=FALSE}
suppressWarnings(library(class))

# find optimal k function, acknowledgements 12
optimal.knn.k <- function(train.data, test.data){
  ...
  opt.k = which.min(mes)
  return(list(opt.k, pl)) #acknowledgements 13
}
```
...
In order to fit the kNN model, I omitted the `Mouse` and `Feedback` columns of the training and testing sets as the `Mouse` variable was not a factor and the `Feedback` is what we are predicting. I also did not use the interaction term `Con.L:Con.R` for this model. 

The kNN model appears to perform better than the LDA and Logistic Regression models. I will also fit a Random Forest model to see if this provides an even greater accuracy rate. 
...
acc = accuracy.rate(cm)
f1 = F1(cm)
print("Random Forest, without Root Spike:")
cat("Model Accuracy:", acc)

# add to summary table, acknowledgements 5
model.sum <- model.sum %>% add_row(Model="Random Forest, with interaction, without root", Accuracy=acc,
                                   Misclassfication.Rate=me, F1.Score=f1)
```

For the Random Forest model, I found that the accuracy was improved with the addition of the `Con.L:Con.R` interaction term, so I included it in this model. 

```{r, echo=FALSE}
model.sum
```

Looking at the model metrics for all the models we have fitted so far, it appears that the Random Forest model has the best performance.However, I want to try re-fitting all of these models with an additional predictor of `Avg.Root` which represents the average spike activity from the `root` brain area. In the data exploration section, I observed a slight difference between the negative and positive feedback types in the four summary sessions, thus, it may help improve the accuracy of the models. I observed this was a brain area observed in all sessions except for 4 and 16, so I will have to omit the data from these sessions. 

To include the `Avg.Root` spike count in my models, I will add a column to my data table which corresponds to the average spike count from the `root` brain area from each trial. Then, I will remove the NAs that will arise from sessions 4 and 16, scale the data, split it into the training and testing sets, and then re-fit and evaluate the resulting models.
...
# new table with avg root
# add column, acknowledgements 8
session.trial.summary.root <- session.trial.summary
session.trial.summary.root$Avg.Root <- rep(0, nrow(session.trial.summary.root))

for(i in 1:nrow(session.trial.summary.root)){
  sid = session.trial.summary.root[i,]$Session
  tid = session.trial.summary.root[i,]$Trial
  ...
  session.trial.summary.root$Avg.Root[i] = avg.root
}

head(session.trial.summary.root)
```

Now that I have added the average root spike count for each trial, I can remove the data with NAs in this column (Sessions 4 and 16), and then scale the data in the column. Once I have completed these pre-processing steps, I can then split the data into the training and testing sets.

```{r, echo=FALSE}
# remove NAs
session.trial.root <- session.trial.summary.root %>% na.omit()

# scale avg. root spikes column
session.trial.root[,9] <- scale(session.trial.root[,9])

# split 70% into training and 30% into testing data sets
set.seed(23)
tot.samples = nrow(session.trial.root)
rand <- sample(1:tot.samples, size=0.7*tot.samples)
train.data.root <- session.trial.root[rand,]
test.data.root <- session.trial.root[-rand,]

# check
print("Training Set:")
head(train.data.root)
print("Testing Set:")
head(test.data.root)

```

Next, I can re-build my models from above with the average root spikes variable as an additional predictor and compare the accuracy rates to see if the average root spike count improves the models' ability to predict trial outcome.

## Linear Discriminant Analysis - With Average Root Spike Count

```{r, echo=FALSE}
mod.root.lda <- lda(formula=Feedback ~., data=train.data.root)
#mod.root.lda

# predict, acknowledgements 3
predict.root.lda <- predict(mod.root.lda, test.data.root)

# confusion matrix
cm <- table(predict.root.lda$class, test.data.root$Feedback)
#cm
...
# add to summary table, acknowledgements 5
model.sum <- model.sum %>% add_row(Model="LDA, with root", Accuracy=acc, Misclassfication.Rate=me, F1.Score=f1)
```

To accurately compare the addition of the `Avg.Root` predictor to the models, I have omitted the `Con.L:Con.R` interaction term from the LDA model. 

## Logistic Regression Model - With Average Root Spike Count

```{r, echo=FALSE}
mod.root.logr <- glm(formula=Feedback~ ., family="binomial", data=train.data.root)
#summary(mod.root.logr)

# predict, acknowledgements 3
predict.root.logr <- predict(mod.root.logr, newdata=test.data.root, type="response")
predict.root.logr <- ifelse(predict.root.logr > 0.5, 1, 0)

# confusion matrix
cm <- table(predict.root.logr, test.data.root$Feedback)
#cm
...
I also omitted the `Con.L:Con.R` interaction term from this logistic regression model including the `Avg.Root` as a predictor since I found that the interaction term did not help the model's accuracy in the earlier logistic regression model without the `Avg.Root` predictor. 
...
# optimal k
opt.k = optimal.knn.k(train.data.root, test.data.root)
opt.k[[2]] # plot
cat("Optimal k:", opt.k[[1]]) # optimal k value

mod.root.knn <- knn(train=train.data.root[-c(3,6)], test=test.data.root[-c(3,6)], 
               cl=train.data.root$Feedback, k=opt.k[[1]])

# confusion matrix
cm <- table(mod.root.knn, test.data.root$Feedback)
#cm
...
rf.root.mod <- randomForest(Feedback ~. +Con.L*Con.R, data=train.data.root, importance=TRUE)

# predict, acknowledgements 3
predict.rf <- predict(rf.root.mod, newdata=test.data.root, type="response")

# confusion matrix
cm <- table(predict.rf, test.data.root$Feedback)
#cm
...
# add to summary table, acknowledgements 5
model.sum <- model.sum %>% add_row(Model="Random Forest, with interaction and root", Accuracy=acc,
                                   Misclassfication.Rate=me, F1.Score=f1)
```

For the random forest model with `Avg.Root` as a predictor, I included the `Con.L*Con.R` interaction term since I included it for the earlier Random Forest model which did not consider the `Avg.Root` as a predictor. 

One last model I want to consider is the decision tree model. However, since its approach is slightly similar to a Random Forest, which uses a collection of decision trees, I will only fit one model for it with the `Avg.Root` as added predictors. However, I could not use the interaction term `Con.L:Con.R` in this model so I omitted it. In doing this, I want to evaluate how much more effective the Random Forest algorithm is compared to a single Decision Tree algorithm in classifying the data accurately. 
...
dtree.mod <- rpart(formula=Feedback~., data=train.data.root, method="class")

# predict, acknowledgements 3
predict.dtree <- predict(dtree.mod, newdata=test.data.root[-6], type="class")

# confusion matrix
cm = table(predict.dtree, test.data.root$Feedback)
#cm
...
# add to summary model table, acknowledgements 5
model.sum <- model.sum %>% add_row(Model="Decision Tree, no interaction with root", Accuracy=acc,
                                   Misclassfication.Rate=me, F1.Score=f1)

```

Now that I have fit various models, it is time to look at the summary table containing all the model metrics for each model and decide upon the best predictive model for this data set. 
...
Looking at this summary table, the Random Forest model with the interaction term `Con.L:Con.R` and without the `Avg.Root` included as a predictor appears to have the best performance based on its high accuracy rate, low misclassification rate, and greatest F-1 score out of all the fitted models. 

The addition of the `Avg.Root` predictor appears to have slightly lowered the accuracy of the LDA model and slightly increased the accuracy of the Logistic Regression model. Perhaps this is due to the omission of data from sessions 4 and 16, which leads to less data being utilized in training the model and can explain the slight differences observed in accuracy. 

In summary, the Random Forest model with interaction and no root is the optimal model for classifying the feedback trial of types. This model utilized `Session`, `Mouse`, `Trial`, `Con.L`, `Con.R`, the interaction term `Con.L:Con.R`, `Avg.Spikes`, and `Max.Spike` as predictors. Since the `Avg.Root` was omitted as a predictor, the data from all sessions was able to be used to train and test the model. 

Next, I will evaluate this model against the test data sourced from Sessions 1 and 18. 

## Evaluate Optimal Model Against Test Data
...
# get session, trial, mouse name, con_left, con_right, feedback, avg. active spikes for each trial across all test sessions

test.summary <- tibble("Session"=as.numeric(), "Trial"=as.numeric(), "Mouse"=as.character(),
                                "Con.L"=as.numeric(), "Con.R"=as.numeric(), "Feedback"=as.numeric(),
                                "Avg.Spikes"=as.numeric(),"Max.Spike"=as.numeric())

for(i in 1:2){
  sid = i
  n.trials = length(test[[sid]]$feedback_type)
  mouse = test[[sid]]$mouse_name
  
  for(j in 1:n.trials){
    tid = j
    con.left = test[[sid]]$contrast_left[[tid]]
    con.right = test[[sid]]$contrast_right[[tid]]
    fb = test[[sid]]$feedback_type[[tid]]
    spikes.nrows = nrow(test[[sid]]$spks[[tid]])
    summed.spk.matrix = apply(test[[sid]]$spks[[tid]], 1, sum)
    total.spks = sum(summed.spk.matrix)
    avg.spks = total.spks / spikes.nrows
    max.spk = summed.spk.matrix[which.max(summed.spk.matrix)] # acknowledgements 4
...
# feedback as factor with levels 0/1
test.summary$Feedback <- ifelse(test.summary$Feedback==-1, 0, 1)
test.summary$Feedback <- test.summary$Feedback %>% factor()

# scale
test.summary[,7:8] <- scale(test.summary[,7:8])

# check
head(test.summary)
```

After reading in the test data and extracting all of the relevant information needed for my optimal Random Forest with interaction, no root model, I can use the model to predict the trial outcome for these test trials.
...
# predict
predict.test <- predict(optimal.mod, newdata=test.summary, type="response")

# confusion matrix
cm <- table(predict.test, test.summary$Feedback)
cm
...
test.mod.metrics <- tibble("Accuracy"=acc, "Misclassification.Rate"=me, "F1.Score"=f1 )
knitr::kable(test.mod.metrics, digits=3) # acknowledgements 10
```

The Random Forest, with interaction and no root model was fairly accurate in predicting the feedback type of each trial. The accuracy percentage means that for 73.5% of trials, the model correctly classified the feedback type of the trial. Consequently, the misclassification rate of 26.5% tells us that 26.5% of trials were classified as the incorrect feedback type. Lastly, the F-1 score represents a combination of precision and recall metrics. Thus, the higher the F-1 score, the higher the precision and recall of the model is. Once again, the precision represents the proportion of cases correctly classified as positive out of all cases classified as positive whereas the recall represents all correctly classified positive cases out of all actual positive cases. Thus, the higher the F-1 score, the better the model.

From the confusion matrix, we can observe that the model seemed to have a more difficult time in classifying the unsuccessful trials as it predicted more of the actual unsuccessful trials as successful than it classified the actual successful trials as unsuccessful. This may be due to the fact that a many of the trials were actually successes. Therefore, the bias towards successful trials made it difficult to train the model to accurately predict the unsuccessful trials as there was less data for it to train these classifications off of. 
...
5. [Add Row to Tibble](https://stackoverflow.com/questions/67531377/inserting-a-new-row-without-specifying-column-names-in-r-using-dplyr)
6. [Fix Overlapping ggplot Labels](https://datavizpyr.com/how-to-dodge-overlapping-text-on-x-axis-labels-in-ggplot2/)
7. [Sort Data Frame](https://www.statology.org/sort-dataframe-by-column-in-r/)
8. [Add Column to Tibble](https://stackoverflow.com/questions/54693682/how-to-add-multiple-columns-to-a-tibble)
9. [Random Forest](https://www.r-bloggers.com/2021/04/random-forest-in-r/)
10. [Models Summary Table](https://bookdown.org/yihui/rmarkdown-cookbook/kable.html)
11. [Table of Contents in R Markdown](https://bookdown.org/yihui/rmarkdown/html-document.html)
12. [Choose Optimal k for kNN](https://medium.com/@moussadoumbia_90919/elbow-method-in-supervised-learning-optimal-k-value-99d425f229e7)
13. [Return More than 1 Value in Function](https://stackoverflow.com/questions/19622028/return-plot-from-r-function)
14. Class Discussion week 8 materials for model metrics
